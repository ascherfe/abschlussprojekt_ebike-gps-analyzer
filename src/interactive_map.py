...
def detect_delimiter(csv_path):
    """
    Erkennt das Trennzeichen anhand der ersten Zeile.
    """
...
def convert_to_float(value):
    """
    Wandelt CSV-Werte sicher in eine Zahl um.
    ...
    # Einheit und unnötige Zeichen entfernen
...
def load_route_data(csv_path):
        ...
        for row_number, row in enumerate(reader, start=1):
            ...
            if elevation_column is not None:
                ...
                if row_number <= 5:
                    print(
                        f"Zeile {row_number}: "
                        f"Rohwert elev = {row.get(elevation_column)!r}, "
                        f"umgewandelt = {elevation}"
                    )
            else:
                elevation = 0.0
            ...
            if elevation is None:
                print(
                    f"Warnung: Höhenwert konnte nicht gelesen werden: "
                    f"{row.get(elevation_column)!r}"
                    )
                elevation = 0.0
    ...
    print()
    print("Erste eingelesene Höhenwerte:")

    for point in route_data[:10]:
